...
import * as _abc from 'underscore';
...
export class Table {
  ...
  public seats: { [idx: number]: Seat };
  public board: any[];
  public deck: Deck | null;
  public button;
  public turn;
  public pot: number;
  public mainPot: number;
  ...
  initSeats(maxPlayers: number): { [idx: number]: Seat } {
    const seats = {};
  ...
  sitPlayer(player: Player, seatId: number, amount: number) {
  ...
  rebuyPlayer(seatId: number, amount: number) {
    if (!this.seats[seatId]) {
  ...
  nextUnfoldedPlayer(player: number, places: number) {
    let i = 0;
    let current = player;
  ...
  nextActivePlayer(player, places: number) {
    let i = 0;
    let current = player;
  ...
  determineWinner(amount, seats) {
    ...
    const findHandOwner = (cards) => {
      const participant = participants.find((participant) =>
        isEqual(participant.solverCards.sort(), cards),
      );
  ...
  dealPreflop() {
    ...
    for (let i = 0; i < 2; i++) {
      for (let j = 0; j < order.length; j++) {
        ...
        if (seat && !seat.sittingOut && this.deck) {
  ...
  handleRaise(socketId: string, amount: number) {
    let seat = this.findPlayerBySocketId(socketId);
